...
X_BOUND = [-5, 5]         # x upper and lower bounds


def F(x):
    # to find the maximum of this function
    return 20 + np.exp(1) - 20 * np.exp((-0.2) * np.sqrt((0.5)*(x**2)))
...
def main():
    ...
    plt.ioff()
    plt.show()
    arquivo = open("Genetic_Algorithm/ackley_GA.txt", "a")
    # arquivo.write("Algoritmo de Algoritmo Genetico com a funcao de Ackley\n\n")

    arquivo.write('The best individual found is {0}'.format(pop[0]) + '\nThe optimal solution is x={0} and f(x)={1} \n\n'.format(
